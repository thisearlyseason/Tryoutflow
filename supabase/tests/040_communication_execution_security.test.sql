...
select has_function('public','authorize_outbox_job_send',array['uuid','uuid','bigint'],'pre-provider authorization boundary exists');
...
select ok(not has_function_privilege('authenticated','public.authorize_outbox_job_send(uuid,uuid,bigint)','execute'),'only service may authorize a provider send');
select ok(has_function_privilege('service_role','public.authorize_outbox_job_send(uuid,uuid,bigint)','execute'),'service must reauthorize immediately before provider');
