...
select ok(has_function_privilege('service_role','public.apply_stripe_subscription_event(text,text,timestamptz,text,text,uuid,text,text,jsonb,text)','execute'),'service role can invoke the narrow verified event RPC');
select ok(not has_function_privilege('authenticated','public.apply_stripe_subscription_event(text,text,timestamptz,text,text,uuid,text,text,jsonb,text)','execute'),'authenticated callers cannot forge verified events');
...
select throws_ok($$insert into public.subscription_events(provider_event_id,event_type,provider_created_at,payload,payload_digest,outcome) values('evt_direct','customer.subscription.updated',now(),'{}',repeat('a',64),'applied')$$,'42501',null,'authenticated direct evidence insert is denied');
reset role;

select is(public.apply_stripe_subscription_event('evt_task24_one','customer.subscription.updated','2026-08-30T12:00:00Z','cus_task24aa','sub_task24aa','51000000-0000-4000-8000-000000000010','team','active','{"verified":true}',repeat('a',64)),'applied','a service-role-shaped verified event applies');
select is(public.apply_stripe_subscription_event('evt_task24_one','customer.subscription.updated','2026-08-30T12:00:00Z','cus_task24aa','sub_task24aa','51000000-0000-4000-8000-000000000010','team','active','{"verified":true}',repeat('a',64)),'replayed','an exact provider event replays');
select is(public.apply_stripe_subscription_event('evt_task24_one','customer.subscription.updated','2026-08-30T12:00:00Z','cus_task24aa','sub_task24aa','51000000-0000-4000-8000-000000000010','club','active','{"verified":false}',repeat('b',64)),'event_conflict','an event ID cannot be rebound to another body');
select is(public.apply_stripe_subscription_event('evt_task24_old','customer.subscription.deleted','2026-08-30T11:59:59Z','cus_task24aa','sub_task24aa','51000000-0000-4000-8000-000000000010','team','canceled','{"old":true}',repeat('c',64)),'ignored_out_of_order','older events never regress state');
...
select is(public.apply_stripe_subscription_event('evt_task24_customer_conflict','customer.subscription.updated','2026-08-30T12:01:00Z','cus_task24aa','sub_task24bb','51000000-0000-4000-8000-000000000020','team','active','{"conflict":true}',repeat('d',64)),'customer_conflict','one provider customer cannot cross organizations');
...
select throws_ok($$update public.subscription_events set outcome='applied' where provider_event_id='evt_task24_old'$$,'55000',null,'event evidence is append-only on update');
select throws_ok($$delete from public.subscription_events where provider_event_id='evt_task24_old'$$,'55000',null,'event evidence is append-only on delete');
