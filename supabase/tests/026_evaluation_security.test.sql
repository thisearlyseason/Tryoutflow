...
select ok(has_function_privilege('authenticated','public.save_evaluation_draft(uuid,uuid,uuid,uuid,uuid,integer,jsonb,text,uuid[],text[])','execute'),'authenticated can invoke guarded draft command');
select ok(not has_function_privilege('anon','public.save_evaluation_draft(uuid,uuid,uuid,uuid,uuid,integer,jsonb,text,uuid[],text[])','execute'),'anonymous cannot save evaluations');
select ok(not has_function_privilege('service_role','public.reopen_evaluation(uuid,uuid,integer,text)','execute'),'service role cannot bypass reopen authorization');
...
select ok(not has_function_privilege('authenticated','public.lock_evaluator_context(uuid,uuid,uuid,uuid,uuid)','execute'),'internal mutation lock helper is not exposed');
select ok(not has_function_privilege('authenticated','public.lock_manager_reopen_context(uuid,uuid,uuid,uuid)','execute'),'internal reopen lock helper is not exposed');
...
select is((select outcome from public.save_evaluation_draft('e1000000-0000-4000-8000-000000000001','e1666666-6666-4666-8666-666666666661','e1000000-0000-4000-8000-000000000014','e1888888-8888-4888-8888-888888888881','e1000000-0000-4000-8000-000000000022',0,'[{"categoryId":"e1000000-0000-4000-8000-000000000023","value":4}]','private note',array['e1000000-0000-4000-8000-000000000031']::uuid[],array['needs_another_look']::text[])),'saved','assigned evaluator creates own draft');
...
select is((select outcome from public.complete_evaluation('e1000000-0000-4000-8000-000000000001',current_setting('app.test.evaluation_a')::uuid,1)),'required_scores_missing','completion never invents a missing score');
select is((select outcome from public.save_evaluation_draft('e1000000-0000-4000-8000-000000000001','e1666666-6666-4666-8666-666666666661','e1000000-0000-4000-8000-000000000014','e1888888-8888-4888-8888-888888888881','e1000000-0000-4000-8000-000000000022',1,'[{"categoryId":"e1000000-0000-4000-8000-000000000023","value":5},{"categoryId":"e1000000-0000-4000-8000-000000000024","value":10}]',null,array[]::uuid[],array[]::text[])),'saved','CAS replaces the entire evaluator-owned draft');
select is((select outcome from public.complete_evaluation('e1000000-0000-4000-8000-000000000001',current_setting('app.test.evaluation_a')::uuid,null)),'conflict','null expected version cannot bypass completion CAS');
select is((select outcome from public.complete_evaluation('e1000000-0000-4000-8000-000000000001',current_setting('app.test.evaluation_a')::uuid,2)),'completed','valid required scores complete atomically');
...
select is((select outcome from public.save_evaluation_draft('e1000000-0000-4000-8000-000000000001','e1666666-6666-4666-8666-666666666661','e1000000-0000-4000-8000-000000000014','e1888888-8888-4888-8888-888888888881','e1000000-0000-4000-8000-000000000022',3,'[]',null,array[]::uuid[],array[]::text[])),'locked','completed record is immutable to evaluator');
select is((select outcome from public.save_evaluation_draft('e1000000-0000-4000-8000-000000000001','e1666666-6666-4666-8666-666666666661','e1000000-0000-4000-8000-000000000014','e1888888-8888-4888-8888-888888888881','e1000000-0000-4000-8000-000000000022',3,'[{"categoryId":"e1000000-0000-4000-8000-000000000099","value":4}]',null,array[]::uuid[],array[]::text[])),'locked','lock state is checked before any replacement');
...
select is((select outcome from public.save_evaluation_draft('e1000000-0000-4000-8000-000000000001','e1666666-6666-4666-8666-666666666661','e1000000-0000-4000-8000-000000000014','e1888888-8888-4888-8888-888888888881','e1000000-0000-4000-8000-000000000099',0,'[]',null,array[]::uuid[],array[]::text[])),'invalid_context','unbound rubric version is rejected');
select is((select outcome from public.save_evaluation_draft('e1000000-0000-4000-8000-000000000001','e1666666-6666-4666-8666-666666666661','e1000000-0000-4000-8000-000000000099','e1888888-8888-4888-8888-888888888881','e1000000-0000-4000-8000-000000000022',0,'[]',null,array[]::uuid[],array[]::text[])),'forbidden','known wrong registration UUID is denied without disclosure');
select is((select outcome from public.save_evaluation_draft('e1000000-0000-4000-8000-000000000001','e1666666-6666-4666-8666-666666666661','e1000000-0000-4000-8000-000000000014','e1888888-8888-4888-8888-888888888899','e1000000-0000-4000-8000-000000000022',0,'[]',null,array[]::uuid[],array[]::text[])),'forbidden','known wrong session UUID is denied without disclosure');
select is((select outcome from public.save_evaluation_draft('e1000000-0000-4000-8000-000000000001','e1666666-6666-4666-8666-666666666661','e1000000-0000-4000-8000-000000000014','e1888888-8888-4888-8888-888888888881','e1000000-0000-4000-8000-000000000022',0,'[{"categoryId":"e1000000-0000-4000-8000-000000000099","value":4}]',null,array[]::uuid[],array[]::text[])),'invalid_score','wrong category is rejected before row creation');
select is((select outcome from public.save_evaluation_draft('e1000000-0000-4000-8000-000000000001','e1666666-6666-4666-8666-666666666661','e1000000-0000-4000-8000-000000000014','e1888888-8888-4888-8888-888888888881','e1000000-0000-4000-8000-000000000022',0,'[{"categoryId":"e1000000-0000-4000-8000-000000000023","value":6}]',null,array[]::uuid[],array[]::text[])),'invalid_score','out-of-range score is rejected');
select is((select outcome from public.save_evaluation_draft('e1000000-0000-4000-8000-000000000001','e1666666-6666-4666-8666-666666666661','e1000000-0000-4000-8000-000000000014','e1888888-8888-4888-8888-888888888881','e1000000-0000-4000-8000-000000000022',0,'[{"categoryId":"e1000000-0000-4000-8000-000000000023","value":4},{"categoryId":"e1000000-0000-4000-8000-000000000023","value":5}]',null,array[]::uuid[],array[]::text[])),'invalid_score','duplicate category cannot address one score twice');
select is((select outcome from public.save_evaluation_draft('e1000000-0000-4000-8000-000000000001','e1666666-6666-4666-8666-666666666661','e1000000-0000-4000-8000-000000000014','e1888888-8888-4888-8888-888888888881','e1000000-0000-4000-8000-000000000022',0,'[]',null,array['e1000000-0000-4000-8000-000000000099']::uuid[],array[]::text[])),'invalid_note_tag','unknown note tag is rejected');
select is((select outcome from public.reopen_evaluation('e1000000-0000-4000-8000-000000000001',current_setting('app.test.evaluation_a')::uuid,3,'Evaluator may not reopen')),'forbidden','evaluator cannot reopen own completion');

select set_config('request.jwt.claim.sub','e1222222-2222-4222-8222-222222222222',true);
select is((select outcome from public.reopen_evaluation('e1000000-0000-4000-8000-000000000001',current_setting('app.test.evaluation_a')::uuid,3,'short')),'invalid_reason','director must provide bounded reason');
select is((select outcome from public.reopen_evaluation('e1000000-0000-4000-8000-000000000001',current_setting('app.test.evaluation_a')::uuid,3,'Review requested after video replay')),'reopened','exact session director reopens completion');
reset role;
...
set local role authenticated;
select set_config('request.jwt.claim.sub','e1333333-3333-4333-8333-333333333333',true);
select is((select outcome from public.save_evaluation_draft('e1000000-0000-4000-8000-000000000001','e1666666-6666-4666-8666-666666666661','e1000000-0000-4000-8000-000000000014','e1888888-8888-4888-8888-888888888881','e1000000-0000-4000-8000-000000000022',4,'[{"categoryId":"e1000000-0000-4000-8000-000000000023","value":3},{"categoryId":"e1000000-0000-4000-8000-000000000024","value":8}]','revised',array[]::uuid[],array[]::text[])),'saved','owner evaluator edits own reopened record');
select is((select outcome from public.complete_evaluation('e1000000-0000-4000-8000-000000000001',current_setting('app.test.evaluation_a')::uuid,4)),'conflict','stale completion loses CAS race');
reset role;
select throws_ok(
...
select is((select outcome from public.save_evaluation_draft('e1000000-0000-4000-8000-000000000001','e1666666-6666-4666-8666-666666666661','e1000000-0000-4000-8000-000000000014','e1888888-8888-4888-8888-888888888881','e1000000-0000-4000-8000-000000000022',0,'[]',null,array[]::uuid[],array[]::text[])),'forbidden','cross-tenant known UUID attack is denied');
...
select is((select outcome from public.complete_evaluation('e1000000-0000-4000-8000-000000000001',current_setting('app.test.evaluation_a')::uuid,5)),'forbidden','offboarded evaluator cannot mutate historical evaluation');
reset role;
select * from finish();
rollback;
