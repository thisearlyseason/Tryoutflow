...
} from '../../../../../src/modules/tryouts/application/save-tryout-setup-step';
import {
  TryoutWizard,
  type TryoutWizardActionState,
} from '../../../../../src/modules/tryouts/ui/tryout-wizard';
...
export default async function WizardFixture({ params }: { params: Promise<{ step: string }> }) {
  ...
  async function fixtureAction(): Promise<TryoutWizardActionState> {
    'use server';
    return { status: 'idle' };
  }
  return (
    <main style={{ maxWidth: 760, margin: '0 auto', padding: 16 }}>
      <WizardProgress completedSteps={['basics', 'divisions']} currentStep={step} hrefBase="" />
      <TryoutWizard
        action={fixtureAction}
        blockers={step === 'review' ? ['rubric_invalid'] : []}
        divisions={[{ id: 'division-1', name: 'U15' }]}
        name="Fall ID Camp"
        sessions={[{ id: 'session-1', name: 'Skills' }]}
        step={step}
      />
    </main>
  );
