...
- Cloudflare Turnstile: `NEXT_PUBLIC_TURNSTILE_SITE_KEY` is the browser widget key; `TURNSTILE_SECRET_KEY` is server-only; `TURNSTILE_ALLOWED_HOSTNAMES` is a comma-separated exact hostname allowlist with no schemes, ports, or paths. Sign-in, account creation, recovery, verification resend, and public registration fail closed if the server configuration or Siteverify response is unavailable, stale, malformed, for another hostname, or for another action.
