...
export type SaveEvaluationGateway = {
  save(input: z.infer<typeof inputSchema> & { expectedVersion: number }): Promise<SaveOutcome>;
};
...
export async function saveEvaluationDraft(
  ...
  try {
    const result = await gateway.save({ ...data, expectedVersion });
