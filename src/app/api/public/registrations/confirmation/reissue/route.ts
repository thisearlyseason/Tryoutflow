  ...
  const guarded = await guardPublicJsonRequest(request, {
    ...
    parse(value) {
      ...
      if (
        ...
        Array.from(body.guardianEmail.trim()).length > 254 ||
        !/^[^\s@]+@[^\s@]+\.[^\s@]+$/u.test(body.guardianEmail.trim())
      )
        return null;
      return {
        body: { token: body.token.toLowerCase(), guardianEmail: body.guardianEmail.trim() },
